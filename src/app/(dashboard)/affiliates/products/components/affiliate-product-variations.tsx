...
// Helpers to build nested points/cost maps
const blankPtsFor = (
  countries: string[],
...
export function AffiliateProductVariations({ attributes, variations, onVariationsChange, warehouses, countries, levels }: Props) {
  const [editingId, setEditingId] = useState<string | null>(null);
  const [expandedId, setExpandedId] = useState<string | null>(null);
  const [skuDraft, setSkuDraft] = useState("");

  // Ensure each variation has full nested points/cost/stock shape
  useEffect(() => {
    ...
    onVariationsChange((cur) =>
      cur.map((v) => {
        ...
        [...Object.keys(prices), ...levels.map((l) => l.id), "default"].forEach((lvl) => {
          ...
          // ensure each country in this level
          countries.forEach((c) => {
  ...
  const generateVariations = () => {
    ...
    const blankStockMap = warehouses.reduce(
      (acc, w) => ({ ...acc, [w.id]: w.countries.reduce((m, c) => ({ ...m, [c]: 0 }), {}) }),
      {}
    );
  ...
  const updateStock = (vid: string, wid: string, c: string, qty: number) =>
    onVariationsChange((cur) =>
      cur.map((v) =>
        v.id === vid
          ? { ...v, stock: { ...v.stock, [wid]: { ...v.stock[wid], [c]: qty } } }
          : v
      )
    );
            ...
            {expandedId===v.id && (
              <CardContent className="p-4 space-y-8">
                ...
                <div className="flex flex-col">
                  <FormLabel className="text-sm mb-1 block">Minimum affiliate level required</FormLabel>
