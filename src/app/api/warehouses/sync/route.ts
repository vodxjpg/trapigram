...
export async function POST(req: NextRequest) {
  try {
    ...
    const { userId, organizationId: activeOrganizationId, tenantId } = ctx;
    ...
    // Non-owner must be a registered recipient
    ...
    // Record recipient’s chosen target warehouse (ignored for owner)
    await db
    ...
    // Enforce tenancy for non-owners
    if (userId !== shareLink.creatorUserId) {
      if (tenantId !== targetWarehouse.tenantId) {
        return NextResponse.json(
    ...
    // Resolve organizationId from warehouse vs current active org
    ...
    if (!warehouseOrgs.length)
      return NextResponse.json(
        { error: "Target warehouse is not linked to an organization" },
        { status: 400 },
      );
    ...
    for (const sp of sharedProducts) {
      ...
      if (productIdMap.has(sp.productId)) {
        ...
        if (reuse) {
          ...
          if (already) {
            ...
            if (!srcProd)
              return NextResponse.json(
                { error: `Source product ${sp.productId} not found` },
                { status: 404 },
              );
            ...
            for (const cat of srcCats) {
              ...
              await db
                ...
                .values({ productId: targetProductId, categoryId: tgtCat.id })
                .execute();
            }

            // copy attributes + terms
            ...
            // copy variations (filtered by share profile)
        ...
        // STOCK aggregation & upsert into target warehouse
        ...
        for (const row of srcStockRows) {
          ...
          if (srcVarId && !variationIdMap.has(srcVarId)) continue;
          ...
          qtyQuery = srcVarId
            ? qtyQuery.where("variationId", "=", srcVarId)
            : qtyQuery.where("variationId", "is", null);
          ...
          let stockQ = db
            ...
            .where("country", "=", row.country);
          stockQ = tgtVarId
            ? stockQ.where("variationId", "=", tgtVarId)
            : stockQ.where("variationId", "is", null);
      ...
      /* ensure mapping row exists (handled above) */
    }
