...
/* ------------------------------------------------------------------
   ...
   – GET    → paginated list with product counts (or all with ?all=1)
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Allow bypassing pagination to fetch the *entire* list (used by selectors)
    ...
    // Count
    let countSql =
      `SELECT COUNT(*) FROM "productCategories" WHERE "organizationId" = $1`;
    const countVals: any[] = [organizationId];
    if (searchTerm) {
      countSql += ` AND (name ILIKE $2 OR slug ILIKE $2)`;
      countVals.push(`%${searchTerm}%`);
    }
    const [{ count }] = (await pool.query(countSql, countVals))
      .rows as [{ count: string }];

    // Rows (with product count)
    let rowsSql = `
      SELECT
        pc.id,
        pc.name,
    ...
    const totalPages = fetchAll ? 1 : Math.ceil(Number(count) / pageSize);
    const currentPage = fetchAll ? 1 : page;

    return NextResponse.json({ categories, totalPages, currentPage });
  } catch (error) {
...
export async function POST(req: NextRequest) {
  ...
  try {
    const { name, slug, image, order, parentId } = categorySchema.parse(
      await req.json()
    );
