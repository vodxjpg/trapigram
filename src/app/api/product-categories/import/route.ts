...
export async function POST(req: Request) {
    ...
    try {
        for (let i = 0; i < data.length; i++) {
            ...
            const rowNumber = i + 3;
            const hasId = category.id && category.id !== "no-id";
            if (hasId) id = category.id

            const findQuery = `SELECT * FROM "productCategories" WHERE id = $1`
            ...
            if (res.length === 0) {
                ...
                const findQuery = `SELECT id FROM "productCategories" WHERE slug = $1`
                const result = await pgPool.query(findQuery, [category.parent]);
                const res = result.rows

                if (res.length > 0) parentId = res[0].id

                await db.insertInto("productCategories")
                    .values({
                        id: uuidv4(),
                        name,
                        slug: category.name,
            ...
            if (res.length > 0) {
                ...
                if (category.name && category.name.trim() !== "") {
                    const name = capitalizeFirstLetter(category.name)
                    updatePayload.slug = category.slug
                ...
                if (category.parent && category.parent.trim() !== "") {
                    ...
                    updatePayload.parentId = res[0].id
                }
