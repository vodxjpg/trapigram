...
import { ensureWallet, getBalances, insertLedgerEntry, findUserIdByExternalIdentity } from "@/lib/credits/db";
...
export async function POST(req: NextRequest) {
  ...
  const { provider, providerUserId, orderId, amount, currency, note } = parsed.data;
  ...
  const { id } = await insertLedgerEntry({
    ...
    amountMinor,
    reason: "purchase",
    reference: { provider, orderId, note: note ?? null },
