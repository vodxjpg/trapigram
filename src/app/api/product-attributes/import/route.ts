...
export async function POST(req: Request) {
    ...
    try {
        for (let i = 0; i < data.length; i++) {
            ...
            const rowNumber = i + 3;
            const hasId = attribute.id && attribute.id !== "no-id";
            if (hasId) id = attribute.id

            const findQuery = `SELECT * FROM "productAttributes" WHERE id = $1`
            ...
            if (res.length === 0) {
                ...
                await db.insertInto("productAttributes")
                    .values({
                        id: uuidv4(),
                        name,
                        slug: attribute.name,
            ...
            if (res.length > 0) {
                ...
                if (attribute.name && attribute.name.trim() !== "") {
                    const name = capitalizeFirstLetter(attribute.name)
                    updatePayload.slug = attribute.slug
