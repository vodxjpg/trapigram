...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        // in src/app/api/affiliate/commands/mylevel/route.ts
        const upsertSql = `
WITH upd AS (
  UPDATE clients
...
-- If the INSERT didn't run, fetch the UPDATE result:
`;
        const { rows } = await pool.query(upsertSql, [orgId, String(userId), username ?? null, firstName ?? null, lastName ?? null]);
        const cli = rows[0];
