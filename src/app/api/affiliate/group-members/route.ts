...
export async function GET(req: NextRequest) {
  ...
  const { groupId, affiliateGroupId, page, pageSize } = qp;

  if (!groupId && !affiliateGroupId)
    return NextResponse.json(
      { error: "Provide groupId or affiliateGroupId query parameter" },
      { status: 400 },
    );

  /* resolve affiliateGroupId if only groupId supplied */
  let agId = affiliateGroupId ?? null;
  if (!agId) {
    const gRow = await db
  ...
  const members = await db
    .selectFrom("affiliateGroupMembers")
    .select(["id", "userId", "clientId", "joinedAt", "createdAt"])
    .where("affiliateGroupId", "=", agId)
