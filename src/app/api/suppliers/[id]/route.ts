...
export async function PATCH(req: NextRequest, ctx: Ctx) {
  ...
  const nextCode =
    has("code") ? String(body.code ?? "").trim() : undefined;
  const nextName =
    has("name") ? String(body.name ?? "").trim() : undefined;
  const nextEmail =
    has("email") ? String(body.email ?? "").trim() : undefined;
  ...
  if (nextCode !== undefined && nextCode !== current.code) {
    const existsQ = await pool.query(
      `SELECT 1 FROM suppliers
        ...
        LIMIT 1`,
      [nextCode, organizationId, supplierId],
    );
    if (existsQ.rowCount) {
  ...
  const sets: string[] = [];
  const vals: any[] = [];
  const add = (columnSql: string, v: any) => {
    vals.push(v);
    sets.push(`${columnSql} $${vals.length}`);
  };
  ...
  `;
  vals.push(supplierId, organizationId);
